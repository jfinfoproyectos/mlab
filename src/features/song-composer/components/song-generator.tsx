...
import { SongLibrary } from '@/features/song-library/components/song-library';
import { PerformanceView } from '@/features/playback/components/performance-view';
import { ProgressionRhythmDialog, type AiRhythmOptions } from '@/features/rhythm-generator/components/progression-rhythm-dialog';
import { SongComposerForm } from "./song-composer-form";
import { SidebarSongLibrary } from '@/features/song-library/components/sidebar-song-library';
import { PlaybackControls } from '@/features/playback/components/playback-controls';
import { ArrangementTimeline } from "./arrangement-timeline";
import { SectionChordEditor } from "./section-chord-editor";
import { PianoRoll } from '@/features/piano-roll/components/piano-roll';
import { AiConfigForm } from '@/features/ai-assistant/components/ai-config-form';
import { AiProgressProvider, useAiProgress } from '@/features/ai-assistant/hooks/use-ai-progress';
import { AiProgressIndicator } from '@/features/ai-assistant/components/ai-progress-indicator';
          ...
          <div className="flex items-center gap-2.5 bg-muted/20 p-1 rounded-2xl border border-border/30 backdrop-blur-sm shadow-sm flex-shrink-0 select-none">
            ...
            {activeSong && (
              <div className="flex items-center gap-2 px-1">
                ...
                {/* MIDI Output Select */}
                <div className="flex items-center gap-1.5 bg-muted/40 border border-border/50 rounded-xl px-2.5 h-8">
                  <Keyboard className={`w-3 h-3 transition-colors ${midiActivity ? 'text-emerald-500 drop-shadow-[0_0_5px_rgba(16,185,129,0.5)]' : 'text-primary'}`} />
                  {isMidiSupported ? (
                    <select
                      value={selectedOutputId}
                      onChange={(e) => setSelectedOutputId(e.target.value)}
                      className="bg-transparent text-[10px] font-bold text-foreground focus:outline-none max-w-[120px] truncate cursor-pointer select-none"
                      title="Dispositivo de Salida MIDI"
                    >
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    className="rounded-xl w-8 h-8 p-0 border-border flex items-center justify-center transition-all bg-background hover:bg-emerald-500/10 hover:border-emerald-500/30 hover:text-emerald-600"
                  >
